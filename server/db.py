...
# Директория для хранения данных MySQL (абсолютный путь)
MYSQL_DATA = Path("mysql_data").resolve()
...
def _start_mysql() -> None:
    ...
    if not (MYSQL_DATA / "mysql").exists():
        subprocess.run([mysqld, "--initialize-insecure", f"--datadir={str(MYSQL_DATA)}"], check=True)

    MYSQL_PROC = subprocess.Popen(
        [
            mysqld,
            f"--datadir={str(MYSQL_DATA)}",
            f"--port={MYSQL_PORT}",
